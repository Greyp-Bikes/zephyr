...
int m41t62_set_time(const struct device *dev, int32_t epoch)
{
	...
	struct tm *time_buffer = localtime(&time_value);
	uint8_t read_buf[M41T62_DATETIME_REG_SIZE] = { 0 };
...
int m41t62_cancel_alarm(const struct device *dev)
{
...
static int m41t62_counter_cancel_alarm(const struct device *dev,
				    uint8_t id)
{
	return -ENOTSUP;
}
